...
class TreeGrid extends React.PureComponent {
  ...
  setStateAndRecalc(state, compute) {
    this.setState(state)
    {
      const e = this.headerRef.current
      if (e) e.recomputeGridSize(compute)
    }
    {
  ...
  renderCell = ({ columnIndex, key, rowIndex, style }, scrollToRow) => {
    ...
    return (<div key={key} className={cc} style={cs} onClick={() => this.onSelectCell({ columnIndex, rowIndex })}>{cell}</div>)
  }
  ...
  render() {
    ...
    return (
      <ResizeSensor onResize={this.onResize}>
        <div className={className} style={{ position: 'fixed', left: 0, top: 0, right: 0, bottom: 0 }} onKeyDown={this.onKeyDown}>
          <Grid
